...
export async function PUT(request: Request): Promise<Response> {
  ...
  if (declared > MAX_FILE_BYTES) return bad(413, `That file is larger than ${MAX_FILE_MB} MB`);
  ...
  const used = await directorySize(dir);
  const remaining = ROOM_QUOTA_BYTES - used;
  if (remaining <= 0) return bad(507, `This huddle has used all ${ROOM_QUOTA_MB} MB of its space`);
  ...
  try {
    ...
    if (error instanceof Error && error.message === "TOO_LARGE") {
      return bad(413, `That file is larger than ${MAX_FILE_MB} MB`);
    }
    if (error instanceof Error && error.message === "QUOTA_FULL") {
      return bad(507, `This huddle has used all ${ROOM_QUOTA_MB} MB of its space`);
    }

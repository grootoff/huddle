...
/**
 * Returns the address other devices on this Wi-Fi should open, plus a QR code for
 * it. The requesting browser already reached us over a working address, so its
 * Host header is the best answer; interface sniffing is the fallback for when that
 * address is localhost, which no other device can use.
 */
export async function GET(request: Request): Promise<Response> {
  const url = new URL(request.url);
  const code = normalizeRoomCode(url.searchParams.get("code") ?? "");

  const forwardedHost = request.headers.get("host") ?? url.host;
  ...
  const origin = `http://${bestHost}${port ? `:${port}` : ""}`;

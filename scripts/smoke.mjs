...
/**
 * The size-limit checks push more than 100 MB per request and fill a room to its
 * 1 GB quota, so they are opt-in: SMOKE_LIMITS=1 npm run smoke. Expect a few
 * minutes and ~1 GB of temporary disk.
 */
const CHECK_LIMITS = process.env.SMOKE_LIMITS === "1";
const { MAX_FILE_MB: CAP_MB, ROOM_QUOTA_MB: QUOTA_MB } = await import("../src/lib/constants.ts");
